...
Game::Game(const char *title, int PositionX, int PositionY, int Width, int Height, bool IsFullscreen,
           const int InFramesPerSecond) : FramesPerSecond(InFramesPerSecond) {
...
void Game::HandleEvents() {
    ...
    while (SDL_PollEvent(&Event)) {
        switch (Event.type) {
            case SDL_QUIT:
                // in case of this event, the game is set to not running and thus the program will proceed to Game::Clean()
                IsRunning = false;
                break;
            case SDL_KEYDOWN:
    ...
    for (int i = 0; i < SignalsCounter; i++) {
        if (!TrafficSignals[i]->GetGreenLight()) {
            for (int j = 0; j < TrainsCounter; j++) {
                if ((TrafficSignals[i]->GetTargetPosition() - Trains[j]->GetCar(0)->GetTransformComponent()->
                     GetPosition()).Abs() < TILE_HEIGHT) {
...
void Game::InitializeTrains(nlohmann::json Data) {
    ...
    for (const auto &trainData: Data["trains"]) {
        bool ShouldMove = trainData["isMoving"];
        ...
        Trains[NumTrains] = NewTrain(GetPointFromID(InitialStationID.c_str()), ShouldMove, Identifier.c_str(),
                                           MaxTrips);
        NumTrains++;
    }
}
...
void Game::InitializeSignals(nlohmann::json Data) {
    ...
    for (const auto &signalData: Data["signals"]) {
        ...
        TrafficSignals[NumSignals] = NewSignal(Coordinates, TargetCoordinates, GreenLight);
        NumSignals++;
    }
}
...
void Game::DeletePoint(RailwayPoint *Point) {
    ...
    for (int j = 0; j < StationsCounter; j++) {
        ...
        for (int i = 0; i < Stations[j]->GetAccessNumbers().y; i++) {
            //Iterate through the Exits
            if (Stations[j]->GetNextPoint(i) == Point) {
                //if it matches the point that we want
...
void Game::HandleHitsUnderCursor() {
    ...
    for (int i = 0; i < TrainsCounter; i++) {
        for (int j = 0; j < NUMBER_OF_CARS; j++) {
            DistanceToCursor = (Trains[i]->GetCar(j)->GetTransformComponent()->GetPosition() - Vector2D<float>{
                                    float(x - TILE_WIDTH / 2), float(y - TILE_HEIGHT / 2)
                                }).Abs();
    ...
    for (int i = 0; i < StationsCounter; i++) {
        if ((Stations[i]->GetTransformComponent()->GetPosition() - Vector2D<float>{
                 float(x - TILE_WIDTH / 2), float(y - TILE_HEIGHT / 2)
             }).Abs() <= TILE_HEIGHT / 2) {
            for (int k = 0; k < TrainsCounter; k++) {
                //check if any train is at the station
                for (int j = 0; j < NUMBER_OF_CARS; j++) {
                    //check per car
                    if ((Trains[k]->GetCar(j)->GetTransformComponent()->GetPosition() - Stations[i]->
                         GetTransformComponent()->GetPosition()).Abs() <= TILE_HEIGHT / 2) {
    ...
    for (int i = 0; i < SignalsCounter; i++) {
        if ((TrafficSignals[i]->GetTransformComponent()->GetPosition() - Vector2D<float>{
                 float(x - TILE_WIDTH / 2), float(y - TILE_HEIGHT / 2)
             }).Abs() <= TILE_HEIGHT / 2) {
            ...
            if (i != j) {
                for (int k = 0; k < NUMBER_OF_CARS; k++) {
                    ...
                    if (IsColliding(*Transform1, *Transform2)) {
                        ...
                        Vector2D<float> PredictedVelocity = (
                            Trains[i]->GetDestinations(0)->GetTransformComponent()->GetPosition() - Transform1->
                            GetPosition()).Normalize();
                        Vector2D<float> PredictedPositionNextTick =
                                Transform1->GetPosition() + PredictedVelocity * GAME_OBJECT_WIDTH;

                        if (IsColliding(TransformComponent{PredictedPositionNextTick, Vector2D<float>{0, 0}, 0.f},
                                        *Transform2)) {
...
bool Game::IsTrainInStation(RailwayPoint *Station) {
    for (int j = 0; j < TrainsCounter; j++) {
        for (int k = 0; k < NUMBER_OF_CARS; k++) {
            if ((Station->GetTransformComponent()->GetPosition() - Trains[j]->GetCar(k)->GetTransformComponent()->
                 GetPosition()).Abs() < GAME_OBJECT_HEIGHT + 10) {
                return true;
            }
        }
    }
